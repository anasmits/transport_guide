...
// Раскомментируйте их по мере работы.
    ...
    void DifferentInputFormat(){
        {
            std::istringstream input{"{\"base_requests\":[],\"stat_requests\":[]}"};
            auto d = Load(input);
            std::cout << "#1  "sv << Print(d.GetRoot()) << std::endl;
        }
        {
            std::istringstream input{"{\"stat_requests\": [],\n\"base_requests\": []}"s};
            auto d = Load(input);
            std::cout << "#2  "sv << Print(d.GetRoot()) << std::endl;
        }
        {
            std::istringstream input{"{\n"
                                     ...
                                     "} "s};
            auto d = Load(input);
            std::cout << "#3  "sv << Print(d.GetRoot()) << std::endl;
        }
    }

    void RunTest() {
        std::cerr << std::endl << "JSON Test starts"s << std::endl;
        TestNull();
        TestNumbers();
        TestStrings();
        TestBool();
        ...
        std::cerr << "Test JSON is passed"s << std::endl;
    }
